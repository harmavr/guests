          ...
          <div className="fixed inset-0 flex items-center justify-end z-10 overflow-auto ">
            <div className="bg-white rounded-lg shadow-lg max-w-lg w-full p-6 space-y-6 h-full">
              ...
              <p className="text-gray-600">
                <i>Fill in the required information for the transaction</i>
              </p>
              ...
              <form className="space-y-4">
                <div className="w-full">
                  <select
                    ...
                    onChange={(e) => {
                      const updatedData = data.items.map((item, index) => {
                        if (index === 0) {
                          return { ...item, paymentMethod: e.target.value };
                        }
                        return item;
                      });
                ...
                <div className="w-full">
                  <select
                    ...
                    onChange={(e) => {
                      const updatedData = data.items.map((item, index) => {
                        if (index === 0) {
                          return { ...item, account: e.target.value };
                        }
                        return item;
                      });
                      setData({ items: updatedData });
                ...
                <input
                  ...
                  onChange={(e) => {
                    const updatedData = data.items.map((item, index) => {
                      if (index === 0) {
                        return { ...item, transactionNumber: e.target.value };
                      }
                      return item;
                    });
                    setData({ items: updatedData });
                ...
                <div className="w-full">
                  ...
                  >
                    ...
                    <option value="Pireus">Pireus</option>
                  </select>
                </div>
