...
import { useAppDispatch } from "@/src/app/[locale]/(content)/lib/hooks";
				...
				complete: function (results) {
					const parsedData = results.data.map(
						(row: any) => ({
							...
							detailedUser: row.detailedUser
								? JSON.parse(row.detailedUser)
								: [],
							tripDetails: row.tripDetails
								? JSON.parse(row.tripDetails)
								: [],
							kidsAges: row.kidsAges
								? JSON.parse(row.kidsAges)
								: [],
						})
					);
